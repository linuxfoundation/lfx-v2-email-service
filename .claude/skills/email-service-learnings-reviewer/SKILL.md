...
This skill is packaged in `lfx-v2-email-service` and may be loaded by a reviewer launched from the LFX workspace root or a multi-repo session. Regardless of the current working directory, it always reviews `lfx-v2-email-service`.

If the caller provides `target repo: lfx-v2-email-service`, use that as confirmation. If the caller provides any other target repo, abort with `INCOMPLETE - lfx-v2-email-service reviewer invoked for <repo>`.
...
- If the repo cannot be found, abort with `INCOMPLETE - lfx-v2-email-service repo not found`.

Run every git command from that repo root.
...
1. **Check `**Detect:**`** — use grep / file reads as the entry directs. Don't infer the match from the `**Pattern:**` description alone; the `**Detect:**` clause is the operational rule. Read the full current file, not just the diff, before concluding a match.
2. **If matched, emit a finding** with:
   ...
   - **Rule:** the entry's full ID (e.g., `tracking-kv-engagement/blind-put-no-optimistic-lock`).
